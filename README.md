...
## Milestone 29
Choose between COD and PayPal Online Payment using radio buttons.

Display PayPal payment buttons when Online Payment is selected.

Securely handle payments using PayPal's API.

Simulated payment process using PayPal's sandbox environment.

## Milestone 30
PayPalScriptProvider: Loads the PayPal JS SDK with the client ID.

PayPalButtons: Renders the PayPal payment buttons.

createOrder: Configures the order with the specified amount.

onApprove: Handles successful payments and shows a confirmation message.

onError: Displays an error message if the transaction fails

## Milestone 31 :- State Management with Redux

In this milestone, we implemented state management using Redux for handling the user's email globally.

✅ Tasks Completed
Installed Redux and React-Redux packages.

Created a store folder with two files: store.js and userActions.js.

Configured a Redux store using configureStore and set up a userReducer to manage the email state.

Created an action called setEmail in userActions.js to update the email in the global state.

Wrapped the entire application inside the Provider component using the Redux store to make the state accessible throughout the app.

This milestone allows efficient state management by maintaining the user's email at a global level, reducing the need for prop drilling and improving overall data handling.
